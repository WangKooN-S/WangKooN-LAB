...
$(window).load(function(){	
	...
	$(".extMenuList li").eq(2).hover(
		function(){
			$(this).stop(true,false).animate({"height":"80px"},300);
...
function mainVisual(){
	...
	function textAnimation(){
		...
		function textWrite(num){
			...
			// 마지막 액션이 실행될때 Skip 버튼을 제거
			if ( num == textLen.length-1 ){
				$("#main").find(".visual").find("span").fadeOut(500);
			}

			// 스킵을 하지 않은 상태일때 텍스트 계속 찍어냄			
			if( !mvSkip ){ // SKIP MODE OFF
				if ( num < $tit.length ){

					$tit.eq(num).css("display","inline-block");

					if( textLen[num] > nowText[num] ){

						$tit.eq(num).css("backgroundPositionY",-nowText[num]*130);
						nowText[num]++;
						...
						if ( num === 2 || num === 1 || num === 4 || num === 5 ){
							if ( num === 1 ){
								$(".titleLine1").find(".cursor").remove();
								$(".titleLine2").find(".cursor").css("display","inline-block");
							}
							if ( num === 4 ){
								$(".titleLine2").find(".cursor").remove();
								$(".titleLine3").find(".cursor").css("display","inline-block");
							}
				...
				$tit.css("display","inline-block");
				$tit.each(
					function(e){
						$(this).css("backgroundPositionY",-(textLen[e]-1)*130);
						$(".titleLine1 .cursor, .titleLine2 .cursor").remove();
						$(".titleLine3 .cursor, .titleLine4").css("display","inline-block");						
						$(".titleLine4").animate({"opacity":1},500);
					}
				);
...
function addSideMenu(){
	...
	$("#wrapper").find(".conBox").each(
		function(e){			
			var $sidelist = $(".sideMenu").find("ul");
			$sidelist.append('<li class="btnSideMenu"><div class="flipMenu">'+sideTit[e]+'</div></li>');
			$sidelist.find(".btnSideMenu").eq(e).css("background-color",sideBgColor[e]);
			$sidelist.find(".btnSideMenu").eq(e).click(function(){scrollMove(e);});
			$sidelist.find("li").eq(e).hover(function(){
...
function scrollMove(num){
	$("body,html").stop().animate({scrollTop: $("#wrapper").find(".conBox").eq(num).offset().top},700);
...
function showSideMenu(){
	...
	var $sidelist = $(".sideMenu").find("ul").find("li");
	
	$sidelist.not(isView).removeClass("on");
	$sidelist.eq(isView).addClass("on");
...
function pageResizing(){
	...
	// 사이즈 메뉴 세로 중앙 정렬
	$(".sideMenuList").css( "margin-top",-( $(".conBox").length * 35 - 10) / 2 );
